...
valid_trips = (df['pickup_latitude'].between(40.5, 41.0)) & \
...
pickup_conditions = [
    (df['pickup_latitude'].between(40.74, 40.77)) & (df['pickup_longitude'].between(-74.00, -73.97)),
...
df['trip_speed_kmh'] = np.where(
    ...
    (df['trip_distance_km'] / df['trip_duration_seconds']) * 3600,
    0
)
...
df['estimated_fare'] = (
    2.50 +
    (df['trip_distance_km'] * 2.50) +
    (df['trip_duration_minutes'] * 0.50)
)
...
df['fare_per_km'] = np.where(
    ...
    df['estimated_fare'] / df['trip_distance_km'],
    0
)
...
expected_time = np.where(
    df['trip_speed_kmh'] > 0,
    df['trip_distance_km'] / df['trip_speed_kmh'] * 3600,
    df['trip_duration_seconds']
)

df['idle_time_ratio'] = np.where(
    df['trip_duration_seconds'] > 0,
    np.clip(1 - (expected_time / df['trip_duration_seconds']), 0, 1),
    0
)
...
required_columns = [
    'id', 'vendor_id', 'pickup_datetime', 'dropoff_datetime', 'pickup_date',
    'pickup_month', 'pickup_hour', 'pickup_day_of_week', 'pickup_day_name',
    'is_pickup_weekend', 'is_pickup_peak_hour', 'time_of_day',
